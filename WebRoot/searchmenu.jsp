...
<%@ page import="po.ManagerCustomer" %>
<%@ page import="po.Tz" %>
<html>
<head>
  <!-- Required meta tags -->
      ...
      <div class="navbar-menu-wrapper d-flex align-items-center justify-content-end">
        <ul class="navbar-nav mr-lg-4 w-100">
         <html:form action="/searchmenu" method="post">
          ...
          padding-right: 22px;
      ">
                <div class="input-group-prepend" style="left: 832px;position: relative;">
                  <button class="input-group-text" id="search">
                    <i class="mdi mdi-magnify"></i>
                  </button>
                </div>
                <input type="text" class="form-control" placeholder="Search now" aria-label="search" aria-describedby="search" name="account">
              </div>
            </li>
          </html:form>
...
　　　　		</form>          
                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="recent-purchases-listing" class="table">
                      ...
                      <tbody>
                       <tr>
                            <th><%=customer.getAccount()%></th>
                            <th><%=customer.getPassword()%></th>
                            <th><%=customer.getCname()%></th>
                            <th><%=customer.getCbalance()%></th>
                            <th><%=customer.getEmail()%></th>
                              <th>
                         
                              <li>
          						<a href="CRUDcustomer.jsp?account=<%=customer.getAccount() %>&password=<%=customer.getPassword() %>
           							&cname=<%=customer.getCname() %>&cbalance=<%=customer.getCbalance() %>&email=<%=customer.getEmail() %>">
              						<span >去修改</span>
            					</a>
      						</li> 
         					 </th>
	...
	for(int i=0;i<customerlist.size();i++){
		Customer stu=(Customer)customerlist.get(i);
	...
	<td><%=stu.getCname() %></td>
	<td><%=stu.getCbalance() %></td>
	<td><%=stu.getEmail() %></td>
	<td>
	  <li>
           <a href="CRUDcustomer.jsp?account=<%=stu.getAccount() %>&password=<%=stu.getPassword() %>
           		&cname=<%=stu.getCname() %>&cbalance=<%=stu.getCbalance() %>&email=<%=stu.getEmail() %>">
              		<span >去修改</span>
            </a>
      </li> 
      </td>
