...
<body>
 <%
 ArrayList tzlist=(ArrayList)request.getSession().getAttribute("tzlist");
...
　　　　		</form>          
	...
	<td>
	  <li>
           <a href="CRUDtz.jsp?name=<%=tz.getName() %>&status=<%=tz.getStatus() %>
           		&office=<%=tz.getOffice() %>&date=<%=tz.getDate() %>&price=<%=tz.getPrice() %>">
              		<span >去修改</span>
            </a>
      </li> 
      </td>
